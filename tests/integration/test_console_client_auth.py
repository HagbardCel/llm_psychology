...
async def test_registration_duplicate_username(test_server_url):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response2 = await client.post(
            f"{test_server_url}/api/auth/register",
            json={"username": username, "password": "different_password", "name": "Different Name"},
        )
...
async def test_login_wrong_password(test_server_url):
    ...
    async with httpx.AsyncClient() as client:
        ...
        data = login_response.json()
        assert "error" in data
        assert "invalid" in data["error"].lower() or "incorrect" in data["error"].lower()
...
async def test_token_based_api_access(test_server_url):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Access /api/auth/me
        me_response = await client.get(f"{test_server_url}/api/auth/me", headers=headers)
        assert me_response.status_code == 200
        assert me_response.json()["username"] == username

        # Access /api/user/status
        status_response = await client.get(
            f"{test_server_url}/api/user/status", params={"user_id": user_id}, headers=headers
        )
        assert status_response.status_code == 200

        # Access /api/sessions
        sessions_response = await client.get(
            f"{test_server_url}/api/sessions", params={"user_id": user_id}, headers=headers
        )
