...
@pytest.mark.trio
async def test_therapy_streams_and_persists_assistant_message(legacy_client):
    """must_preserve: legacy chat streams token chunks and persists its assistant turn."""
...
async def test_therapy_lifecycle_closes_session_and_revises_plan(legacy_client):
    ...
    plans_before = len(legacy_client.server.rows("therapy_plans"))
    ...
    assert therapy_row.get("session_summary") or therapy_row.get("enriched") == 1

    plans_after = legacy_client.server.rows("therapy_plans")
    assertions.assert_plans(plans_after, minimum=plans_before + 1)
    assert len(plans_after) == plans_before + 1
