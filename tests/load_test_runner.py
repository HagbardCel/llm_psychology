...
class LoadTestRunner:
    ...
    def setup_test_environment(self):
        ...
        class LoadTestConfig:
            ...
            MIGRATIONS_DIR = self.temp_migrations_dir
            GOOGLE_API_KEY = "load_test_api_key"
            MODEL_NAME = "gemini-2.5-flash"
            DOMAIN_KNOWLEDGE_PATH = self.temp_domain_dir
            VECTOR_DB_PATH = self.temp_vector_dir
            DATABASE_POOL_SIZE = max(10, self.num_workers * 2)  # Ensure adequate pool size
        
        self.config = LoadTestConfig
        
        print(f"Load test environment setup:")
    ...
    def setup_container(self):
        ...
        mock_llm = Mock()
        mock_llm.generate_response.return_value = "Mock therapeutic response for load testing"
        mock_llm.generate_structured_response.return_value = {
            'raw_response': '{"key_themes": ["stress", "anxiety"], "emotional_state": "concerned", "insights": ["recognizing_patterns"], "progress_indicators": ["self_awareness"]}'
        }
        ...
        mock_rag.retrieve_relevant_knowledge.return_value = [
            ...
            {"content": "Mindfulness approaches for anxiety", "source": "mindfulness.md"}
        ]
    ...
    def simulate_user_session(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            test_session = Session(
                ...
                transcript=[
                    Message(role="user", content="I've been feeling overwhelmed with work stress lately", timestamp=datetime.now()),
                    Message(role="assistant", content="Can you tell me more about what's been stressing you?", timestamp=datetime.now()),
                    Message(role="user", content="I think I need better coping strategies for deadlines", timestamp=datetime.now())
                ]
            )
            ...
            plan_assessment = planning_agent.assess_plan_effectiveness(therapy_plan)
            recommendations = planning_agent.recommend_plan_adjustments(therapy_plan)
            ...
            therapeutic_insights = reflection_agent.get_therapeutic_insights()
    ...
    def run_load_test(self) -> Dict[str, Any]:
        ...
        """
        print(f"\nStarting load test with {self.num_users} users and {self.num_workers} workers...")
        ...
        # Analyze results
        return self.analyze_results(total_execution_time, initial_memory, final_memory, initial_cpu, final_cpu)
    
    def analyze_results(self, total_time: float, initial_memory: float, final_memory: float, 
                       initial_cpu: float, final_cpu: float) -> Dict[str, Any]:
    ...
    def print_report(self, analysis: Dict[str, Any]):
        ...
        def get_emoji(rating: str) -> str:
            return {'Excellent': '🟢', 'Good': '🟡', 'Acceptable': '🟠', 'Poor': '🔴'}.get(rating, '❓')
        
        print(f"  Response Time:     {get_emoji(assessment['response_time'])} {assessment['response_time']}")
        print(f"  Reliability:       {get_emoji(assessment['reliability'])} {assessment['reliability']}")
        print(f"  Memory Efficiency: {get_emoji(assessment['memory_efficiency'])} {assessment['memory_efficiency']}")
        print(f"  Overall Rating:    {get_emoji(assessment['overall'])} {assessment['overall']}")
        ...
        if assessment['overall'] == 'Excellent':
            ...
            if assessment['memory_efficiency'] in ['Acceptable', 'Poor']:
                print("     - Review memory usage patterns and optimize data structures")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Load test the psychoanalyst application')
    ...
    parser.add_argument('--quick', action='store_true', help='Run quick test with fewer users')
    ...
    try:
        ...
        if analysis['success'] and analysis['performance_assessment']['overall'] in ['Excellent', 'Good']:
            exit_code = 0
        else:
            exit_code = 1
