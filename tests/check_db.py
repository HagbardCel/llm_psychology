...
def check_database(db_path):
    ...
    try:
        ...
        if count > 0:
            cursor.execute("SELECT session_id, user_id, timestamp FROM sessions LIMIT 5")
            rows = cursor.fetchall()
            for row in rows:
                print(f"  - Session ID: {row[0]}, User ID: {row[1]}, Timestamp: {row[2]}")
        else:
        ...
        if count > 0:
            cursor.execute("SELECT plan_id, user_id, created_at, version FROM therapy_plans LIMIT 5")
            rows = cursor.fetchall()
            for row in rows:
                print(f"  - Plan ID: {row[0]}, User ID: {row[1]}, Created: {row[2]}, Version: {row[3]}")
        else:
        ...
        if count > 0:
            cursor.execute("SELECT user_id, name, created_at FROM user_profiles LIMIT 5")
            rows = cursor.fetchall()
            for row in rows:
