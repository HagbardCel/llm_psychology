...
def check_database(db_path):
    ...
    try:
        ...
        if count > 0:
            cursor.execute("SELECT session_id, user_id, timestamp, transcript FROM sessions")
            ...
            for i, row in enumerate(rows):
                ...
                transcript_preview = row[3][:100] + "..." if len(row[3]) > 100 else row[3]
        ...
        if count > 0:
            cursor.execute("SELECT plan_id, user_id, created_at, updated_at, plan_details, version FROM therapy_plans")
            rows = cursor.fetchall()
            for i, row in enumerate(rows):
                print(f"  Plan {i+1}:")
        ...
        if count > 0:
            cursor.execute("SELECT user_id, name, birthdate, profession, created_at FROM user_profiles")
            rows = cursor.fetchall()
            for i, row in enumerate(rows):
                print(f"  Profile {i+1}:")
