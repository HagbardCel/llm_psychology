...
class _DummyInput:
    async def read(self, prompt: PromptSpec) -> str:
        raise AssertionError("input should not be read")
...
@pytest.mark.asyncio
async def test_human_input_provider_delegates_to_prompt_session() -> None:
    session = MagicMock()
    session.prompt_async = AsyncMock(return_value="hello")
    provider = HumanInputProvider(session=session)

    result = await provider.read(PromptSpec(text="> "))

    assert result == "hello"
    session.prompt_async.assert_awaited_once_with("> ")


@pytest.mark.asyncio
async def test_human_input_provider_propagates_eof() -> None:
    session = MagicMock()
...
@pytest.mark.asyncio
async def test_human_input_provider_propagates_keyboard_interrupt() -> None:
    session = MagicMock()
...
@pytest.mark.asyncio
async def test_human_input_provider_is_cancellable() -> None:
    async def hang(_text: str) -> str:
        await asyncio.Event().wait()
        return "never"

    session = MagicMock()
    session.prompt_async = hang
    provider = HumanInputProvider(session=session)
...
def test_cli_maps_keyboard_interrupt_to_exit_130() -> None:
    ...
    with patch("jung.client.terminal._async_cli", interrupted):
        assert cli() == 130
...
async def test_async_cli_passes_transport_timeout_to_settings() -> None:
    ...
    with (
        ...
        patch(
            "jung.client.terminal.HumanInputProvider",
            return_value=_DummyInput(),
        ),
        patch(
...
async def test_run_console_maps_exit_codes() -> None:
    ...
    with patch("jung.client.terminal.JungApiClient", return_value=mock_client):
        with patch(
            ...
            assert await run_console(settings, input_provider=dummy_input) == 0

        with patch(
            "jung.client.terminal.ConsoleApp.run",
            AsyncMock(side_effect=ConsoleOperationFailed()),
        ):
            assert await run_console(settings, input_provider=dummy_input) == 1
        ...
        with patch(
            ...
            output = TerminalConsoleOutput()
            assert (
                await run_console(settings, input_provider=dummy_input, output=output)
                == 3
            )
