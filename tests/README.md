...
| Session/supervisor LLM role routing | `integration/application/test_llm_role_composition.py` — dual adapters, effective AdapterConfig resolution, lifecycle task ownership | `unit/llm/test_policies.py` for role→model policy mapping; `unit/llm/test_tracing.py` for physical gateway role in diagnostics |
