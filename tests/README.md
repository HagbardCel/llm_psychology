...
| Session review + grounding persistence | `integration/store/test_store_workflow.py` — patient citation → authoritative source-session Message FK, review round-trip, ordering, rollback | `integration/application/` one representative case proving processor review reaches the store and appears through `list_grounded_patient_messages()`; `PhaseInputs` owns prior-review selection and latest-briefing handoff |
| Citation semantics | `unit/phases/post_session/test_evidence_validation.py` — sequence existence, role, chronology, visibility, uniqueness | none at higher layers |
| Bounded context projection | `unit/phases/therapy/test_context.py`, `unit/phases/post_session/test_post_session_prompts.py`, `unit/phases/post_session/test_update_context.py`, `unit/phases/test_context_projection.py` — budget reservation, trust/priority semantics, forbidden legacy prompt keys | none at higher layers |
...
including patient endpoint resolution, `patient_metrics` roll-up, and the
phase-local Phase 8D benchmark mechanics). Those tests run in `make check`. The
live multi-session real-model journey is manual and is **not** part of
`make check`.
