...
DEFAULT_RELEVANCE_TERMS = ("monday", "deadline", "chest", "failing", "ceiling", "sleep")
...
async def run_assertions(recorder: Any, scenario: dict[str, Any]) -> bool:
    ...
    for action in criteria.get("required_actions", ["start_intake", "select_therapy_style", "start_therapy", "continue_therapy"]):
    ...
    await check(
        ...
        any(
            ...
            and event.get("data", {}).get("workflow_state") == "plan_update_in_progress"
            for event in recorder.events
        ),
    )
    await check(
    ...
    intake_user_turns = [item for item in intake_transcript if item.get("role") == "user"]
    await check(
    ...
    if plan_rows:
        ...
        current_plans = [row for row in plan_rows if row.get("superseded_by_plan_id") is None]
        ...
        await check("therapy_plan_version_incremented", int(latest_plan.get("version") or 0) >= 2)
        await check("single_current_plan", len(current_plans) == 1)
        await check(
            "explicit_plan_lineage",
            bool(
                previous_plans
                and latest_plan.get("supersedes_plan_id") == previous_plans[-1].get("plan_id")
                and previous_plans[-1].get("superseded_by_plan_id") == latest_plan.get("plan_id")
            ),
        )
        await check(
            ...
            bool(
                therapy_sessions
                and previous_plans
                and therapy_sessions[0].get("plan_id") == previous_plans[-1].get("plan_id")
            ),
        )
        await check(
        ...
        await check("therapy_plan_briefing_persisted", bool(latest_plan.get("session_briefing")))
        await check(
            "profile_linked_to_latest_plan",
            bool(profile_rows and profile_rows[0].get("plan_id") == latest_plan.get("plan_id")),
        )
        briefing = json.loads(latest_plan.get("session_briefing") or "{}")
        await check(
    ...
    therapy_jobs = [
        ...
        if therapy_sessions and row.get("session_id") == therapy_sessions[0].get("session_id")
    ]
    await check(
    ...
    relevance_terms = criteria.get("therapy_response_relevance_terms", DEFAULT_RELEVANCE_TERMS)
    matched_terms = [
    ...
    await check(
        ...
        any(item.get("agent") == "THERAPIST" for item in therapy_transcript if item.get("role") == "assistant"),
    )
    await check(
        ...
        any(
            ...
            and event.get("data", {}).get("agent_type") == "THERAPIST"
            and event.get("data", {}).get("selected_therapy_style") == "cbt"
            for event in recorder.events
        ),
    )
