...
async def run_probe(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if any(event["kind"] == "session_ended" for event in recorder.events):
            await wait_for_plan_update(args.db_path, user_id, scenario, recorder)
...
async def wait_for_plan_update(db_path: str, user_id: str, scenario: dict, recorder: ProbeRecorder) -> None:
    timeout = float(scenario.get("limits", {}).get("plan_update_timeout_seconds", 120))
    with trio.move_on_after(timeout):
        while True:
            state = await trio.to_thread.run_sync(partial(_profile_state, db_path, user_id))
...
def _profile_state(db_path: str, user_id: str) -> str | None:
    if not Path(db_path).exists():
        return None
    with sqlite3.connect(db_path) as conn:
        row = conn.execute("SELECT status FROM user_profiles WHERE user_id = ?", (user_id,)).fetchone()
