...
try:
    ...
    _output_path = Path(__file__).resolve().parent / "output.py"
    _spec = importlib.util.spec_from_file_location("console_output", _output_path)
    if _spec is None or _spec.loader is None:
        raise
    _module = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_module)
    ConsoleOutput = _module.ConsoleOutput
