...
|---|---|---|---|
| `profile` | Singleton identity/preferences; method fixed at Finish Intake; current-plan pointer set by application | Product defaults or explicit user input plus application pointer | Language/name editable while idle; method editable only during intake |
...
**Profile:** `singleton_id=1`, optional display name, primary language and preferred style initialized to English/supportive, current plan ID, created/updated timestamps. Initialize profile and intake atomically, with no SETUP flag or required confirmation. Defaults are not evidence of explicit choice. The current pointer is application-authored; model-generated biography never enters the profile. Idle intake preference edits update its snapshot until Finish Intake freezes it atomically. Intake closure also makes the method immutable for all later sessions, including failed initial review/retry; derive that restriction from closure without another flag. Language/name remain editable. Therapy snapshots freeze language at creation; later language edits apply to future sessions. Closed-session review retries retain their original preferences.
...
3. Include mandatory core: fixed method/session language, complete compact plan/handoff, current input, active handoff sources, and the preceding session's unanswered patient input. If optional R6a is justified and implemented, include explicit recall sources too. Deduplicate by message ID; no priority rule silently drops a mandatory source to fit another.
