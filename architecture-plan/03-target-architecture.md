...
| Stage | Commands |
|---|---|
| INTAKE | Chat without a selected method; choose/change method and edit language while idle; finish after a nonblank patient message and a valid catalog method choice |
...
The profile is the sole durable owner of `method`; each session stores only a scalar `language`, not `preferences_json` or a method copy. During intake, method is initially null; user selection/change must resolve to a packaged method ID before the application writes it. Reject unknown IDs without mutation; no model selects the method. Language edits atomically update `profile.current_language` and the open intake's `language`. Language edits affect subsequent replies without rewriting past messages. Method edits only change the selection for initial review and later therapy; intake prompt construction never consumes that selection. Finish Intake validates the nonblank patient contribution and selected catalog method under command ownership before closing intake and queuing review in one transaction. This makes method non-null/immutable and freezes intake language. An edit racing with closure is serialized by normal command ownership. Reject method changes after closure, including while initial review is pending or failed; do not silently accept or queue them. The closed intake establishes this invariant without another lock flag or SETUP stage.

The target profile response exposes nullable `method` during intake; the method-edit input accepts a known packaged ID. An omitted method leaves the selection unchanged; clearing a selected method is not a required feature. Missing selection at Finish Intake returns an actionable command error and preserves the open intake/transcript without scheduling review or making a model call. Unknown persisted IDs, missing method assets, or null method after closure are explicit catalog/invariant errors; never substitute another method or silently repair state.
...
1. Console displays English, “method not selected,” the existing catalog descriptions, and a short orientation: concerns, impact/time course, goals, coping, and safety can be discussed; unknowns and declining to answer are acceptable. Explain that a method must be selected before Finish Intake, becomes immutable then, and first affects initial planning and subsequent therapy rather than intake replies.
2. Intake already exists with editable method/language and no initial plan. A static welcome invites the first patient contribution; it is UI guidance, not fabricated therapist dialogue. Selection is available throughout intake, but no explicit method/language confirmation is required to start chatting.
3. Patient text uses the ordinary durable chat path. The conversation task always uses the common conversation/orientation/safety policy and session language, never the selected method or its instructions. Selecting or changing a method does not change intake prompts. It asks concise follow-ups based on the recent transcript. There is no extraction call, extra selection call, or durable inferred intake record.
...
5. **Finish intake** is an explicit user command. It does not certify clinical completeness or safety. Accept it only after at least one nonblank patient message and a valid selected catalog method, even if clinical questions remain. If either is missing, preserve the open intake and explain the unmet requirement. Success freezes the profile method and intake session language, closes the session, and queues review in one transaction. A trailing unanswered message remains source material for that review.
6. The initial review is the first model call to consume the frozen method. It reads the full method-neutral intake and produces a provisional plan and handoff. It records unknowns and questions for later clarification. A valid initial review requires a plan; it does not require a diagnosis or style score.
7. Commit and enter `READY`. No assessment operation, all-styles assessment-output coverage validator, or post-assessment style-selection stage remains. Validation that the selected method resolves to the packaged catalog remains required.
...
Empty therapy sessions take a deterministic no-change path. Therapy with only an unanswered patient contribution also takes a deterministic review indicating that it was not explored; its source is retained and presented at the next session. Do not infer disengagement. Intake with patient text and a selected valid method still needs the initial review even if no assistant completed, because it must establish a provisional plan. Empty intake or intake without a selected method cannot finish.
