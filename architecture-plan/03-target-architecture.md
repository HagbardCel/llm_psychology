...
1. Console displays English, “method not selected,” the existing catalog descriptions, and a short orientation: concerns, impact/time course, goals, coping, and safety can be discussed; unknowns and declining to answer are acceptable. Explain that a method must be selected before Finish Intake and becomes immutable then.
...
3. Patient text uses the ordinary durable chat path. With no method selected, the conversation task uses only the common conversation/orientation/safety policy and session language. After selection it also uses the packaged method instructions for subsequent replies. It asks concise follow-ups based on the recent transcript. There is no extraction call, extra selection call, or durable inferred intake record.
