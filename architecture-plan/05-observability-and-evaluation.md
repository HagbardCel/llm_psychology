...
Ordinary logs plus current product state cannot reproduce every historical prompt after intake language edits, code upgrades, or retention rotation. This is an explicit limit. Small artifact provenance identifies the generator; opt-in capture establishes exact inputs when required.
...
- A short common safety/boundary instruction applies across intake, therapy methods, and review. Method instructions may guide the therapeutic approach but never replace that common policy.
...
This proposal makes no claim that a single model, two models, or a particular therapy method is clinically validated. Clinical effectiveness and crisis response quality need a separate appropriately designed evaluation, not more runtime agents.
...
R0b follows the development/confirmation protocol in the [migration plan](06-migration-plan.md). R0a commits confirmation requirements, not fixtures accessible to the tuning actor. After candidate/rubric freeze, rerun the complete development set. Only after every case passes does a separate evaluator create fresh cases without candidate outputs; fix cases and expectations before execution. Alternatively, pre-authored fixtures must stay outside the tuning actor's accessible working set until the frozen development rerun passes. Run confirmation once each on the unchanged candidate. Require both review modes for each packaged method in development and all three methods plus both modes collectively in confirmation, within the existing case budget. Only actual model review calls count; catalog tests and deterministic no-op paths cannot satisfy coverage. Record resolved temperature/sampling and reasoning settings, exact review-method assets, per-case method ID/review mode/instruction digest, evaluator identity, fixture digest, and freeze/development-pass/confirmation order. Missing coverage or an instruction digest mismatch leaves admission pending; this gate does not admit therapy conversation guidance. Retain failed evidence and use fresh confirmation cases after any findings-driven revision. Human review checks small-plan-change preservation and use of the method fixed at intake closure, not post-intake transitions. These few cases support a bounded architectural decision, not statistical reliability or clinical safety.
...
- Catalog unit: all packaged IDs have nonempty description/conversation/review assets; missing or empty required assets fail explicitly; legacy assessment/therapist/reflection files are unnecessary. Therapy and review use their distinct guidance fields; intake uses neither.
...
- API/console: English/unselected initialization and nullable profile response, chat under common policy regardless of selection, catalog selection/change during intake, unknown method rejection without mutation, Finish rejection for missing method or empty intake without closing/scheduling, atomic profile-method freeze/edit races, rejection of later method changes including failed initial review, scalar session language unaffected by later profile language edits, interrupted stream/retry, basic inspection, capacity warning/draft preservation. Intake prompts are identical for otherwise identical inputs across null and all valid method selections; language edits still affect reply language. Therapy/review resolve their respective packaged instructions from the profile method, without a session/plan copy or fallback. The final intake selection first affects initial review. Complete browsing/selection/recall behavior is tested only if optional R6a is justified and implemented.
- Eval harness: fixture isolation, patient information boundary, nonzero failure outcome, cancellation/capture failure, and no evidence-success on missing files. R0 coverage checks reject missing methods, missing development method/mode combinations, incomplete confirmation coverage, instruction digest mismatches, and deterministic shortcuts counted as model reviews.
...
| Method-instruction canary not copied into generated artifacts | Retain one live review case, with method instructions separated from sources |
...
4. Keep stable instructions early in prompts so runtime prefix caches can help; Jung does not own a KV-cache layer. Cache only immutable method text/schema objects in process if useful.
