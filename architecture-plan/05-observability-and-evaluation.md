...
- Context unit/store integration: identical canonical source serialization for accounting/review, multilingual/escaping/many-short-turn boundaries, retries count once, mandatory sources, contiguous exchanges, temporal labels, no older review notes, and chronology-only candidate ordering. R6's 100-session fixture separates mandatory-source inclusion from unanchored misses and measures bounded queries/serialization. Only optional R6a adds recall metadata and revalidates the envelope.
- API/console: default intake, idle preference edits until closure, atomic method-freeze/edit races, rejection of later method changes including failed initial review, stable language snapshots/retries, interrupted stream/retry, basic inspection, capacity warning/draft preservation. Complete browsing/selection/recall behavior is tested only if optional R6a is justified and implemented.
