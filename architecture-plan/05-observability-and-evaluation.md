...
R0b follows the development/confirmation protocol in the [migration plan](06-migration-plan.md). Withhold confirmation inputs, expected findings, and baseline outputs from tuning; final admission reruns all development cases and opens confirmation only after freezing the candidate and rubric. Retain failed evidence and use fresh confirmation cases after any findings-driven revision. Human review explicitly checks small-plan-change preservation and method transitions. These few cases support a bounded architectural decision, not statistical reliability or clinical safety.
