...
| R2b — `feat/r2b-conversation-workflow` | Build shared free-text intake/therapy, initialized defaults, intake edits until atomic finish/freeze, method immutable after intake closure, therapy language snapshots at creation, explicit end/finish and pending review state. Test the new application behavior directly; keep the existing public composition working until R2d can wire the complete replacement. |
...
- Fresh initialization is idempotent with no SETUP gate; intake edits after early messages affect subsequent replies/final plan; finish/edit races freeze exactly one final snapshot and method. Later method changes are rejected, including during failed initial review; retries preserve the original snapshot. Therapy language freezes at creation and later language edits remain pending.
- Conversation receives the complete plan/handoff under the fixed method, without projection or another call. Plan method provenance derives from its source review session. Empty-session handling remains no-change.
...
**Documentation / exit:** record measurements, material misses, and acceptable unanchored limits. Fix failures of existing mandatory-source contracts directly; they do not justify a retrieval feature. If the baseline meets the named needs, proceed to R7 with no recall DTO, metadata, selection UI, or extra schema reset. If a material gap remains, use the same cases to test whether explicitly supplying the missing source would remedy it before selecting R6a. Record the rationale for proceeding or deferring; do not redefine a required failure as acceptable. Dependency: R5.
...
- [ ] R0a designates a runtime, development cases, and confirmation requirements; after candidate freeze a separate evaluator creates untouched confirmation cases. R0b passes both sets on the frozen R1-based candidate within one correction per case. Human comparison includes unrelated plan-content preservation; resolved sampling/reasoning settings, mode/envelope decisions, authorship/freeze order, and failures are recorded. Additional advertised runtimes have separate admission.
