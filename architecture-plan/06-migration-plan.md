...
Select 6–10 frozen synthetic completed sessions covering initial plan, no change, later correction, denial without broadening to an unasked dimension, historical/current attribution, plan change, and injection. Include starting plan, handoff, dated patient sources, and a method change during intake. Compare useful two-pass output on matched source inputs, reusing applicable frozen output or obtaining a bounded baseline after R1. If the old packer supplied different sources, disclose that coverage difference rather than claiming a matched comparison.
...
Run all frozen cases on the designated runtime, capturing exact synthetic request/result, physical attempts, validation, and human findings directly. At most one correction; transport/protocol failures are not correction retries. Require every case to produce useful valid output within its budget and satisfy its hard source contracts. Failures are not averaged away. Refine schema/prompt/capacity and rerun affected cases; a measured two-pass solution to persistent failures requires revising D2 before R2, never a competing production flag.
...
Base `feat/architecture-cutover` on main containing R1 and the admitted R0b inputs. Create the stack below; the first PR compares against that base, subsequent PRs against their immediate predecessor. Owning deterministic tests accompany every layer. Intermediate branches are review artifacts, not supported releases; only the complete validated integration targets main.

| Step / branch | Responsibility and review boundary |
|---|---|
| R2a — `feat/r2a-schema-store` | Five-table domain/SQL model and transactional primitives. Bump schema version (7 on inspected main); remove operations/intake JSON. Purpose-free references and plan-owned source-review link; no reverse plan ID, speculative input metadata, capacity JSON, or work framework. Store tests land here. |
| R2b — `feat/r2b-conversation-workflow` | Shared free-text intake/therapy, initialized defaults, intake edits until atomic finish/freeze, therapy snapshots at creation, explicit end/finish and pending review state. Application ownership tests accompany the flow. |
...
| R2d — `feat/r2d-api-console` | Remove SETUP/assessment/style ranking and generic operation retry; add session review retry and explicit intake end. Show defaults/preferences, failures, basic history/source and derived plan relationships, capacity warning/draft preservation, and explicit help. Preserve NDJSON reconciliation. Chat is text-only; R6 owns recall DTO/storage/UI. |
| R2e — `feat/r2e-remove-old-contracts` | Remove superseded phases/owners and actual main-based intermediate assertions; integrate replacement live source/negation/next-context contracts and adapted journey. Verify the combined stack and docs. R2e is not the first testing step, and R3 cannot defer surviving correctness checks. |
...
- [ ] R0a designates a runtime and freezes cases; R0b passes every case on R1's corrected boundary within one correction, with human comparison and mode/envelope decisions. Additional advertised runtimes have separate admission.
