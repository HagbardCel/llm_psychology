...
Promote the exact R0b-admitted review-method guidance too; a packaging/name change alone does not invalidate evidence, but changes to supplied instruction content require revalidation of affected properties. The target catalog in [target architecture](03-target-architecture.md) requires description, conversation, and review assets only. R2c can introduce it alongside the old catalog without routing production through both; R2d removes old fields/assets when their last callers are cut over, and R2e verifies no legacy loader requirement survives.
