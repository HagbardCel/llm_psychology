...
Freeze 6–8 development cases with IDs, source inputs, and expected findings, plus the coverage requirements/rubric for 3–4 later confirmation cases. Development covers initial plan, no change, later correction, denial without broadening to an unasked dimension, historical/current attribution, and injection; cases can cover more than one property. Include starting plan, handoff, dated patient sources, and a method edit during method-neutral intake (CBT to Jung) whose final value first governs initial planning, without generating method-specific intake turns. Post-intake method switching is outside scope. Include a focused plan-change case: multiple existing goals and an important caution remain relevant, while the session justifies changing only one approach item within the fixed method. The full replacement must preserve unrelated relevant content; structural validity alone cannot detect its loss.

Distribute the development cases across every packaged method's exact `review_instructions` and both review modes. The six required cases are:

| Case | Method ID | Review mode | Primary coverage |
|---|---|---|---|
| D1 | `jung` | Initial | Initial plan; final intake selection governs review |
| D2 | `cbt` | Initial | Denial preservation and source attribution |
| D3 | `freud` | Initial | Injection resistance and uncertainty |
| D4 | `jung` | Later | Small plan change preserves unrelated goals/cautions |
| D5 | `cbt` | Later | Correction and historical/current attribution |
| D6 | `freud` | Later | No-change decision and stale-source attribution |

Use D7/D8 only for uncovered requirements, not a full scenario × method expansion. The 3–4 confirmation cases must represent `jung`, `cbt`, and `freud` at least once each, with initial and later review both represented across the set; each method need not appear in both confirmation modes. Freeze these coverage requirements before tuning. Every counted case must execute a model review with the designated method guidance, including the later no-change case on a conversational session. Deterministic empty-session shortcuts and catalog-loading tests do not count. This admits review guidance, not therapy conversation guidance, which retains its separate behavioral validation.

In R0a, commit only confirmation requirements, not confirmation fixtures in the tuning checkout. After R0b candidate freeze **and a passing complete development rerun**, a separate evaluator/person/agent receives those requirements and input/output contracts, without candidate outputs or tuning transcripts, and constructs fresh cases plus expected findings. Freeze those fixtures before executing the candidate or baseline on them. The tuning actor makes no further candidate changes after seeing them. If cases are instead prepared in advance, keep them outside the tuning actor's accessible working set until that development rerun passes; a directory label in the same accessible repository is insufficient. Record evaluator identity, fixture digest, candidate-freeze revision, and development-pass/confirmation order in the small evidence manifest. For each case, record method ID, review mode, and the digest of the exact review-instruction content supplied; bind those digests to the frozen candidate assets. No benchmark framework or encryption is needed. If that separation cannot be arranged, confirmation remains pending rather than relabeling inspected cases.
...
Designate one required local review model/server/build/configuration **before** final tests. Target `json_schema`; additional runtime implementations are separate support-admission work. Record candidate source/non-session/output limits, prompt/schema versions, and exact review-method guidance. Use the target `review_instructions` content in the prototype; shared review-task policy stays outside method assets. Do not select a winner after silently dropping failed cases or promise both llama.cpp and MTPLX by default.

**Rubric:** supported interpretation, visible uncertainty, useful handoff and provisional/changed plan, correct attribution, no denial broadening or obvious method/boundary conflict. Structural validity alone does not establish quality or clinical safety. Test committed prototype/capture mechanics deterministically, including rejection of missing methods, missing development method/mode combinations, incomplete confirmation coverage, mismatched instruction digests, and deterministic shortcuts counted as model calls. Use the small manifest rather than a new benchmark framework. Run `make check`.
...
**Exit:** completed live admission and human comparison on one designated runtime for all three packaged review methods: both modes per method in development, every method and both modes collectively in confirmation. Record R1 revision, schema/prompt, per-case method/mode/instruction digest, configuration, structured-output mode, and envelope. An unavailable required runtime leaves R0b pending and blocks R2, not R1. Each additional advertised runtime needs separate admission; optional incompatibility neither blocks canonical cutover nor automatically adds a production mode. Any exception needs a named required endpoint and measured need. No production schema changes occur in R0a/R0b.
...
| R2b — `feat/r2b-conversation-workflow` | Build shared free-text intake/therapy, English/unselected initialization, method-neutral intake chat regardless of selection, catalog-valid selection/change as metadata only until finish, atomic finish requiring a nonblank patient contribution and valid method. Method becomes non-null/immutable after closure; therapy language freezes at creation. Test new application behavior directly; keep existing public composition working until R2d can wire the complete replacement. |
| R2c — `feat/r2c-review-context` | R0b-admitted draft, source resolution, initial-plan/no-op rules, atomic completion, one owned task/recovery/fencing, data-directory lock before recovery. One serializer for accounting/review; complete plan/handoff under the fixed method, anchors, unanswered input, recent references and complete exchanges. Introduce `jung.methods` with separate conversation/review guidance, using R0b-admitted review assets; retain legacy assets while old public callers need them. No method projection, purpose ranking, older review notes, lexical search, or explicit recall. |
| R2d — `feat/r2d-api-console` | Switch public composition, API, and console together to the complete R2b/R2c flow. Remove SETUP/assessment/style ranking and generic operation retry; add session review retry and explicit intake end. Replace affected old tests in this same layer. Expose nullable method, display English/unselected state and existing catalog choices, explain finish requirements, method immutability and its first use in initial planning, pending language edits, failures, basic history/source and derived plan relationships, capacity warning/draft preservation, and explicit help. Preserve NDJSON reconciliation. Chat is text-only; optional R6a owns any future recall DTO/storage/UI. Delete superseded persistence owners and legacy `jung.styles` fields/assets/package with their final callers; no compatibility alias remains. |
| R2e — `feat/r2e-remove-old-contracts` | Remove residual unreachable phases/helpers and main-based intermediate assertions; integrate replacement live source/negation/next-context contracts and adapted journey. Verify final five-table ownership, combined stack, and docs. Verify retirement of `jung.styles` and remove any unreachable legacy catalog residue; no assessment/reflection assets remain required solely by the old loader. R2e is not the first testing step or a place to repair a knowingly broken R2d, and R3 cannot defer surviving correctness checks. |
...
Promote the exact R0b-admitted review-method guidance for all three methods, checking per-method instruction digests against the admission manifest; a packaging/name change alone does not invalidate evidence, but changes to supplied instruction content require revalidation of affected properties. The target catalog in [target architecture](03-target-architecture.md) requires description, conversation, and review assets only. R2c can introduce `jung.methods` alongside legacy `jung.styles` without routing production through both; R2d removes old fields/assets when their last callers are cut over, and R2e verifies no legacy loader requirement survives.
...
- Catalog loading covers every packaged ID with nonempty description/conversation/review assets, rejects missing or empty required assets explicitly, and succeeds without legacy assessment/therapist/reflection prompt files. Target consumers use only their appropriate instruction field; common policy is not duplicated into method assets.
- Fresh initialization is idempotent with English and null method, no SETUP gate, and chat always uses common intake policy regardless of selection. Parameterize valid method selection/change over packaged IDs; unknown values including `supportive` are rejected without mutation. Method edits after early messages leave intake prompts unchanged and affect only the eventual initial plan/therapy; language edits affect subsequent replies. Finish/edit races either reject unmet preconditions with no closure or freeze exactly one valid profile method and intake language. Later method changes are rejected, including during failed initial review. Therapy language freezes at creation; later profile language edits leave existing sessions/retries unchanged and apply to the next session.
- Therapy conversation and review read method only from the profile and language from the session. For otherwise identical language/transcript/input, intake prompts are identical across null and every valid method selection; no selected-method metadata or instructions are injected. A CBT-to-Jung intake edit first affects initial review, which uses Jung review guidance; therapy uses Jung conversation guidance. Unknown persisted IDs, missing assets, or null method after closure fail explicitly without fallback. There is no session method copy, generic preferences JSON, or plan-method property, projection, or extra model call. Empty-session handling remains no-change.
...
- [ ] No old task modes, legacy `jung.styles` package/alias, operation result owners, duplicate plan recommendations, forensic event dependencies, or speculative configuration/retrieval systems remain.
