...
- post-session completion validates grounded message IDs (unique; role `user`; same source session) then inserts references, writes `review_json`, optionally creates a plan revision, and completes the operation in one transaction;
