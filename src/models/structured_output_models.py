...
class Tier2Enrichment(BaseModel):
    ...
    dominant_affects: list[str] = Field(..., min_items=1)
    key_themes: list[str] = Field(..., min_items=1)
...
class Tier4Extract(BaseModel):
    ...
    initial_goals: list[str] = Field(..., min_items=1)
    current_progress: str = Field(..., min_length=1, max_length=2000)
    planned_interventions: list[str] = Field(..., min_items=1)
