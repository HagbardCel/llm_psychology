...
def _evidence_drop_reason(
    ...
    *,
    ...
    if evidence.source_message_sequence != source_message_sequence:
...
def _merge_evidence_list(
    existing: tuple[IntakeEvidence, ...],
    patches: tuple[IntakeEvidence, ...],
    *,
    max_length: int,
) -> tuple[IntakeEvidence, ...]:
    merged = list(existing)
    seen = {key for item in merged if (key := _evidence_list_key(item))}
...
def _merge_validated_patch(
    ...
    if patch.safety:
        merged = merged.model_copy(
            update={
                "safety": merged.safety.model_copy(
                    update={
                        "self_harm": _merge_evidence(
                            merged.safety.self_harm, patch.safety.self_harm
                        ),
                        "harm_to_others": _merge_evidence(
                            merged.safety.harm_to_others, patch.safety.harm_to_others
                        ),
                        "medical_urgency": _merge_evidence(
                            merged.safety.medical_urgency,
                            patch.safety.medical_urgency,
                        ),
                    }
                )
            }
    ...
    if patch.coping:
        merged = merged.model_copy(
            update={
                "coping": merged.coping.model_copy(
                    update={
                        "attempted_strategies": _merge_evidence_list(
                            merged.coping.attempted_strategies,
                            patch.coping.attempted_strategies,
                            max_length=20,
                        ),
                        "substances_or_medication": _merge_evidence(
    ...
    if patch.goals:
        merged = merged.model_copy(
            update={
                "goals": merged.goals.model_copy(
                    update={
                        "therapy_goals": _merge_evidence_list(
                            merged.goals.therapy_goals,
                            patch.goals.therapy_goals,
                            max_length=10,
                        ),
                        "preferred_start": _merge_evidence(
