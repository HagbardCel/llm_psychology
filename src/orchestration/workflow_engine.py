"""
Workflow engine for managing therapy workflow state transitions.

This module implements a state machine that manages user progression
through the therapy workflow stages.
"""
...
class WorkflowEngine:
    """
    State machine for therapy workflow management.
    ...
    VALID_TRANSITIONS: Dict[WorkflowState, List[WorkflowState]] = {
        ...
        WorkflowState.ASSESSMENT_COMPLETE: [WorkflowState.THERAPY_IN_PROGRESS],
    ...
    WORKFLOW_STATE_TO_USER_STATUS: Dict[WorkflowState, UserStatus] = {
        ...
        WorkflowState.INTAKE_IN_PROGRESS: UserStatus.PROFILE_ONLY,
        WorkflowState.INTAKE_COMPLETE: UserStatus.INTAKE_COMPLETE,
        WorkflowState.ASSESSMENT_IN_PROGRESS: UserStatus.INTAKE_COMPLETE,
        WorkflowState.ASSESSMENT_COMPLETE: UserStatus.PLAN_COMPLETE,
        WorkflowState.THERAPY_IN_PROGRESS: UserStatus.PLAN_COMPLETE,
        WorkflowState.REFLECTION_IN_PROGRESS: UserStatus.PLAN_COMPLETE,
    ...
    def __init__(self, db_service: DatabaseService):
        ...
        Args:
            db_service: Database service for state persistence
        """
    ...
    async def transition(
        self, user_id: str, new_state: WorkflowState, event: Optional[WorkflowEvent] = None
    ) -> None:
        """
        ...
        # Update database
        await self.db_service.update_user_status(user_id, user_status)

        logger.info(f"User {user_id} transitioned to {new_state} (status: {user_status})")
