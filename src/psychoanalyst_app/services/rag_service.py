...
class RAGService:
    ...
    def __init__(
        ...
        *,
        ...
        if styles_dir is _DEFAULT_STYLES_SENTINEL:
            resolved_styles = Path(resources.files("psychoanalyst_app") / "styles")
        elif styles_dir is None:
            resolved_styles = None
        else:
            styles_path = Path(cast(str | Path, styles_dir)).expanduser()
            if not styles_path.is_absolute():
                styles_path = (Path.cwd() / styles_path).resolve()
