"""
TrioMemoryAgent: Trio-native specialized agent for managing session context and therapeutic memory.
...
from psychoanalyst_app.models.structured_output_models import SessionAnalysis
from psychoanalyst_app.prompts.memory_prompt_builder import build_session_context_prompt
...
class TrioMemoryAgent:
    ...
    def __init__(
        ...
        """
        Initialize the Trio Memory Agent.

        Args:
            llm_service: LLM service for content analysis (synchronous)
            db_service: Trio database service for session retrieval
    ...
    async def analyze_session_context(self, session: Session) -> SessionContext:
        """
        Analyze a session to extract key contextual information using Trio.
    ...
    async def get_therapeutic_memory(self, refresh: bool = False) -> TherapeuticMemory:
        """
        Get comprehensive therapeutic memory for the user using Trio.
        ...
        Raises:
            MemoryError: If memory building fails
        """
        ...
        try:
            ...
            for session in sessions:
                try:
                    ...
                    logger.warning(
                        f"Failed to analyze session {session.session_id}: {e}"
                    )
                    continue
    ...
    async def get_recent_context(self, num_sessions: int = 3) -> dict[str, Any]:
        """
        Get context from recent sessions for immediate therapy planning using Trio.
        ...
        try:
            ...
            for session in recent_sessions:
                try:
                    ...
                    logger.warning(
                        f"Failed to analyze recent session {session.session_id}: {e}"
                    )
                    continue
    ...
    async def identify_patterns(self) -> dict[str, Any]:
        """
        Identify patterns and trends across all sessions using Trio.

        Returns:
            Dict containing identified patterns

        Raises:
            MemoryError: If pattern identification fails
        """
        ...
        try:
            ...
            progress_patterns = self._analyze_progress_patterns(
                memory.progress_timeline
            )
    ...
    async def get_continuity_context(self, current_session_topics: list[str]) -> str:
        """
        Get context for maintaining continuity with current session using Trio.
    ...
    def _analyze_progress_patterns(
    ...
    def _generate_context_summary(
        self, themes: list[str], emotions: list[str], insights: list[str]
    ) -> str:
    ...
    async def health_check(self) -> bool:
        """
        Perform health check on the memory agent using Trio.
        ...
        try:
            ...
            # Test LLM service if sessions exist (run in thread)
            if sessions:
            ...
            logger.error(f"TrioMemoryAgent health check failed: {e}")
            return False
