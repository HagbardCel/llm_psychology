"""
PlanningAgent: Specialized agent for therapy plan creation and adjustment.
...
from exceptions import AgentError, PlanningError
...
class PlanEvolution:
    ...
    def __init__(self, plan_id: str, version: int, changes: List[str], 
                 rationale: str, effectiveness_score: float = 0.0):
...
class PlanningStrategy:
    ...
    def __init__(self, therapy_style: str, focus_areas: List[str], 
                 techniques: List[str], assessment_criteria: List[str]):
...
class PlanningAgent:
    """
    Agent specialized in therapy plan creation and strategic adjustments.
    ...
    def __init__(self, llm_service: LLMService, db_service: DatabaseService, 
        ...
        # Planning state
        self.current_strategy: Optional[PlanningStrategy] = None
        self.plan_evolution: List[PlanEvolution] = []
        
        logger.info(f"PlanningAgent initialized for user {user_context.user_id}")
    
    def create_initial_plan(self, intake_session: Session, 
                          selected_style: Optional[str] = None) -> TherapyPlan:
        """
        Create comprehensive initial therapy plan.
        ...
        try:
            ...
            session_context = self.memory_agent.analyze_session_context(intake_session)
            
            # Get relevant domain knowledge
            session_text = self._extract_session_text(intake_session)
            relevant_knowledge = self._get_relevant_knowledge(session_text, selected_style)
            
            # Determine therapy style if not specified
            if not selected_style:
                selected_style = self._recommend_therapy_style(session_context, relevant_knowledge)
            ...
            plan_details = self._generate_initial_plan_details(
            ...
            success = self.db_service.save_therapy_plan(therapy_plan)
            if not success:
    ...
    def update_plan(self, session: Session, current_plan: TherapyPlan, 
                   force_update: bool = False) -> TherapyPlan:
        """
        ...
        try:
            ...
            # Get relevant knowledge for update
            ...
            # Generate updated plan details
            updated_details = self._generate_updated_plan_details(
                session, session_context, memory, current_plan, relevant_knowledge
            )
            
            # Identify specific changes made
            changes = self._identify_plan_changes(current_plan.plan_details, updated_details)
            ...
            # Save updated plan
            success = self.db_service.save_therapy_plan(updated_plan)
            if not success:
            ...
            evolution = PlanEvolution(
                ...
                changes=changes,
                rationale=self._generate_update_rationale(session_context, memory, changes)
            )
            self.plan_evolution.append(evolution)
    ...
    def assess_plan_effectiveness(self, plan: TherapyPlan) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            recent_context = self.memory_agent.get_recent_context(num_sessions=3)
    ...
    def recommend_plan_adjustments(self, plan: TherapyPlan) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            effectiveness = self.assess_plan_effectiveness(plan)
            ...
            technique_recommendations = self._recommend_technique_adjustments(plan, memory)
    ...
    def get_plan_evolution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_strategy': {
                'therapy_style': self.current_strategy.therapy_style if self.current_strategy else None,
                'focus_areas': self.current_strategy.focus_areas if self.current_strategy else []
            }
        }
    ...
    def _get_relevant_knowledge(self, session_text: str, therapy_style: Optional[str]) -> List[Dict[str, Any]]:
        """Get relevant domain knowledge filtered by therapy style."""
        if therapy_style and style_service.get_style_pack(therapy_style):
            ...
            if knowledge_source:
                return self.rag_service.retrieve_relevant_knowledge(
                    session_text, n_results=3, filter_source=knowledge_source
                )
        
        return self.rag_service.retrieve_relevant_knowledge(session_text, n_results=3)
    ...
    def _create_planning_strategy(self, therapy_style: str, session_context) -> PlanningStrategy:
        ...
        if style_config:
            ...
            assessment_criteria = ['emotional_progress', 'behavioral_changes', 'insight_development']
        else:
    ...
    def _generate_initial_plan_details(self, intake_session: Session, session_context, 
...
"""
    ...
    def _generate_updated_plan_details(self, session: Session, session_context, memory, 
        ...
        Dominant Themes: {', '.join(memory.recurring_themes.keys())[:3] if memory.recurring_themes else 'None'}
...
"""
        ...
        updated_details.update({
            ...
            'memory_insights': recent_context.get('insights', [])[-2:],  # Last 2 insights
    ...
    def _parse_plan_response(self, response: Dict[str, Any], 
                           strategy: Optional[PlanningStrategy]) -> Dict[str, Any]:
        ...
        default_details = {
            ...
            'techniques': ', '.join(strategy.techniques if strategy else ['supportive_therapy']),
            'themes': ', '.join(strategy.focus_areas if strategy else ['general_wellbeing']),
    ...
    def _assess_update_necessity(self, session_context, memory, current_plan: TherapyPlan) -> bool:
    ...
    def _identify_plan_changes(self, old_details: Dict[str, Any], 
                             new_details: Dict[str, Any]) -> List[str]:
    ...
    def _generate_update_rationale(self, session_context, memory, changes: List[str]) -> str:
        ...
        return "; ".join(rationale_parts) if rationale_parts else "Routine plan update based on session progress"
    
    def _calculate_effectiveness_score(self, plan: TherapyPlan, memory, 
        ...
        quality_scores = {'new': 0.0, 'building': 0.1, 'developing': 0.2, 'established': 0.3, 'strong': 0.4}
        score += quality_scores.get(relationship_quality, 0.0)
        
        # Clamp score between 0.0 and 1.0
        return max(0.0, min(1.0, score))
    
    def _generate_effectiveness_assessment(self, plan: TherapyPlan, memory, 
                                         recent_context: Dict[str, Any], 
    ...
    def _recommend_theme_adjustments(self, plan: TherapyPlan, 
        ...
        for theme in dominant_themes[:2]:  # Top 2 themes
            if theme not in current_themes:
                recommendations.append({
                    ...
                    'rationale': f"Theme appears frequently in recent sessions",
                    'priority': 'medium'
                })
        
        return recommendations
    
    def _recommend_technique_adjustments(self, plan: TherapyPlan, memory) -> List[Dict[str, Any]]:
    ...
    def _recommend_goal_adjustments(self, plan: TherapyPlan, 
                                  effectiveness: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            plans = self.db_service.get_latest_therapy_plan(self.user_context.user_id)
            ...
            logger.error(f"PlanningAgent health check failed: {e}")
            return False
    
    def __str__(self) -> str:
        """String representation of planning agent."""
        return f"PlanningAgent(user={self.user_context.user_id}, style={self.current_strategy.therapy_style if self.current_strategy else 'none'})"
    
    def __repr__(self) -> str:
        """Detailed representation of planning agent."""
        evolution_count = len(self.plan_evolution)
        return f"PlanningAgent(user='{self.user_context.user_id}', evolutions={evolution_count})"